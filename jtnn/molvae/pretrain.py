...
for i in range(len(dataset.data)):
    if (i + 1) % 100 == 0:
        print(i)
    mol_tree = dataset.__getitem__(i)
    if len(mol_tree.nodes) != 0:
        new_data.append(dataset.data[i])
    else:
        print dataset.data[i]
print 'size of new dataset: %d' % len(new_data)

f = open('../data/new_data.txt','w')
for x in cset:
    f.write(str(x)+'\n')
f.close()
new_dataset = MoleculeDataset('../data/new_data.txt')
'''
MAX_EPOCH = 5
PRINT_ITER = 20
